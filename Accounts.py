...
class InventoryPage:
    def __init__(self, account_window):
        ...
        def upgrade():
            conn = sqlite3.connect("./Database/CoffeeShop.db")
            cur = conn.cursor()
            cur.execute("INSERT INTO Employee_Account (employee_fullname, employee_username, employee_password) VALUES (?, ?, ?)",
            (employee_fullname.get(), employee_username.get(), employee_password.get())) 
            conn.commit()
            conn.close()
            show_all_employee()
            clear_employee()
        ...
        def show_all_employee():
            ...
            if len(rows) != 0:
                ...
                for index, row in enumerate(rows, start=1):
                    employee_id = row[1]
                    # Inserting the employee ID as the first column
                    employee_tree.insert('', END, values=(index, employee_id) + row[2:])
                conn.commit()
            conn.close()
        ...
        employee_tree.heading("#1", text="EmployeeID", anchor=W)
        employee_tree.heading("#2", text="Employee Full Name", anchor=W)
        employee_tree.heading("#3", text="Username", anchor=W)
        employee_tree.heading("#4", text="Password", anchor=W)
        ...
        self.button3.configure(background="#ff6600")
        ...
        self.button6.configure(background="#e60000")
        ...
        self.button7.configure(background="#ff0066")
        ...
        self.button8.configure(background="#0073e6")
        ...
        self.button9.configure(background="#ff0066")
